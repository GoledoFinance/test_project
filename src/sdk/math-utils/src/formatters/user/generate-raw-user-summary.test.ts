...
import { generateRawUserSummary, RawUserSummaryResponse } from './generate-raw-user-summary';
import {
...
describe('generateRawUserSummary', () => {
  ...
  it('should generate the correct user summary ', () => {
    ...
    expect(normalize(rawSummary.totalLiquidityUSD, marketReferenceCurrencyDecimals)).toEqual(
      '40000'
    );
    expect(normalize(rawSummary.totalCollateralUSD, marketReferenceCurrencyDecimals)).toEqual(
      '40000'
    );
    expect(normalize(rawSummary.totalBorrowsUSD, marketReferenceCurrencyDecimals)).toEqual('20000');
    expect(
      normalize(rawSummary.totalLiquidityMarketReferenceCurrency, marketReferenceCurrencyDecimals)
    ).toEqual('4000');
    expect(
      normalize(rawSummary.totalCollateralMarketReferenceCurrency, marketReferenceCurrencyDecimals)
    ).toEqual('4000');
    expect(
      normalize(rawSummary.totalBorrowsMarketReferenceCurrency, marketReferenceCurrencyDecimals)
    ).toEqual('2000');
    expect(
      normalize(rawSummary.availableBorrowsMarketReferenceCurrency, marketReferenceCurrencyDecimals)
    ).toEqual('0');
  ...
  it('should generate the correct user summary for user in stablecoin eMode', () => {
    ...
    expect(normalize(rawSummaryEMode.availableBorrowsUSD, marketReferenceCurrencyDecimals)).toEqual(
      '4000'
    );
