...
import { ReserveIncentiveMock, ReserveMock, UserIncentiveMock, UserReserveMock } from '../../mocks';
...
describe('calculateAccruedIncentives', () => {
  const reserveMock = new ReserveMock().addLiquidity(100).addVariableDebt(200).addStableDebt(300);
  const userMock = new UserReserveMock().supply(100).variableBorrow(200).stableBorrow(300);
  const reserveIncentiveMock = new ReserveIncentiveMock();
  const userIncentiveMock = new UserIncentiveMock();
  const currentTimestamp = 1;

  const { totalLiquidity, totalVariableDebt, totalStableDebt } = calculateReserveDebt(
    reserveMock.reserve,
    currentTimestamp
  );
  ...
  const stableDebtRewardsRequest: CalculateAccruedIncentivesRequest = {
    principalUserBalance: new BigNumber(userMock.userReserve.principalStableDebt),
    reserveIndex: new BigNumber(
    ...
    precision: 18,
    reserveIndexTimestamp:
      reserveIncentiveMock.reserveIncentive.sIncentiveData.rewardsTokenInformation[0]
        .incentivesLastUpdateTimestamp,
    emissionPerSecond: new BigNumber(
  ...
  it('should default to reserveIndex if rewards emission is 0', () => {
    ...
    expect(normalize(result, 18)).toBe(normalize(stableDebtRewardsRequest.reserveIndex, 18));
  });
