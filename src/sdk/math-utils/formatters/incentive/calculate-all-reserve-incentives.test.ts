...
describe('calculateAllReserveIncentives', () => {
  ...
  const userReserveMock = new UserReserveMock().supply(100).variableBorrow(100).stableBorrow(100);
  ...
  it('calculates correct incentives data for each reserve asset', () => {
    ...
    expect(result['0x0000000000000000000000000000000000000000'].aIncentives[0].incentiveAPR).toBe(
      '105120'
    );
    expect(result['0x0000000000000000000000000000000000000000'].vIncentives[0].incentiveAPR).toBe(
      '315360'
    );
    expect(result['0x0000000000000000000000000000000000000000'].sIncentives[0].incentiveAPR).toBe(
      '0'
    );
  });
